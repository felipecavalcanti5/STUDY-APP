...
export const CartoesEstudoProvider = ({ children }) => {
    ...
    const atualizarCartoes = async (id, atualizacoes) => {
        const novosCartoes = cartoes.map(cartao => cartao.id ===id ? {
                ...cartao, ...atualizacoes} : cartao)
        setCartoes(novosCartoes)
        await AsyncStorage.setItem('cartoes', JSON.stringify(novosCartoes))
    }
    ...
    return (
        <CartoesEstudoContext.Provider value={{ cartoes, adicionarCartao, atualizarCartoes, excluirCartao}}>
            { children }
